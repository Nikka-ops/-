...
- **小红书（主源）** — 默认 `Playwright + 已登录 CDP Chrome`：驱动真实浏览器搜索、**监听浏览器自发的搜索响应**，不重放签名请求、不烧 Cookie；`Spider_XHS` 作兼容回退。
- **牛客（辅源）** — 自动发现面经帖，二次请求详情页补全正文。
- **图片帖** — 下载图片 → RapidOCR 读文字 → 合并入正文；OCR 失败/低质时可调 **视觉大模型**（Qwen-VL / GLM-4V，配 `VISION_API_KEY`）兜底读图。
...
**MIT License** · © 2026 · Built with [Claude Code](https://claude.com/claude-code)
