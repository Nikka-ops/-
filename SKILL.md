...
- `scripts/connectors/xiaohongshu.py` → `XiaohongshuConnector(export_path=..., driver=...).search(queries) -> SearchResult`(二选一:`export_path` 读预生成的 JSON;`driver` 自动跑 MediaCrawler)
- `scripts/scrape/mediacrawler_driver.py` → `MediaCrawlerDriver(home=None).scrape_xhs(keywords) -> Path`,**驱动模式**——shell out 调本机已装的 MediaCrawler(默认从 `$MEDIACRAWLER_HOME` 或 `~/.mediacrawler/` 找),前提是用户已经扫码登录过一次。
...
0. **准备(仅当启用小红书源)。** 两种模式二选一,见 `docs/setup/mediacrawler.md`:
   - **driver 模式(推荐)**:用户**一次性**装 MediaCrawler 并扫码登录。之后 skill 用 `XiaohongshuConnector(driver=MediaCrawlerDriver())` 自动跑采集,登录态过期前都不需要再管。
